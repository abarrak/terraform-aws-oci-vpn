...
variable "oci_ipsec_gateway_cpe_name" {
  type        = string
  description = "(Required) the name for the aws customer gateway for the oci ipsec tunnel."
  default     = "aws-to-oci-cpe"
}

variable "oci_ipsec_gateway_cpe_ip" {
  type        = string
  description = "(Required) the public IP for the aws customer gateway. This is provided from the counterpart created in oci."
}

variable "oci_ipsec_vpn_gateway_name" {
  type        = string
  description = "(Required) the name for the aws vpn gateway."
  default     = "aws-vpn-virtual-gateway"
}

variable "oci_ipsec_vpn_connection_name" {
  type        = string
  description = "(Required) the name for the aws vpn connection."
  default     = "AWS-To-OCI-Connection"
}

variable "oci_ipsec_vpn_connection_routes" {
  type        = list(string)
  description = "List of one or more static route between a VPN connection and a customer gateway."
  default     = []
}

variable "oci_ipsec_vpn_connection_tunnel_encryption_algorithms" {
  type        = list(string)
  description = "(Optional) List of one or more encryption algorithms that are permitted for the VPN tunnel's IKE negotiations."
  default     = ["AES256"]
}

variable "oci_ipsec_vpn_connection_tunnel_integrity_algorithms" {
  type        = list(string)
  description = "(Optional) List of one or more encryption algorithms that are permitted for the VPN tunnel's IKE negotiations."
  default     = ["SHA2-256"]
}

variable "oci_ipsec_vpn_connection_tunnel_dh_group_numbers" {
  type        = list(string)
